...
- key_actions：从上方流年数据中选2-3个年份，key=年份数字，value=该年具体行动建议。必须按维度选择不同十神的年份：
  * career 事业：选官杀（正官/七杀）透出之年，或印星（正印/偏印）护官之年
  * wealth 财富：选财星（正财/偏财）透出之年，或食伤（食神/伤官）生财之年
  * marriage 婚姻：男命选财星（正财/偏财）透出之年，女命选官杀（正官/七杀）透出之年
  * relationship 贵人：选印星（正印/偏印）透出之年，或用神年
  * health 健康：选忌神年，或流年冲克日主之年
- 建议要具体可操作，不要泛泛而谈
...
def _build_forecast_prompt(
    ...
    # ===== 按维度分别选 key_years（核心规则） =====
    lines.append("\n## key_years 按维度分别选年规则（每个维度必须选出不同的2-3个年份）")
    lines.append("根据《子平真诠·论妻财子禄》和《渊海子平·六亲》的十神对应原则，各维度应选不同十神的流年：")
    lines.append("◆ career 事业：从流年中筛选「十神」为【正官/七杀/正印/偏印】的年份作为 key_years，这些是官禄和权力关键年。若无官印流年，则选用神年。")
    lines.append("◆ wealth 财富：从流年中筛选「十神」为【正财/偏财/食神/伤官】的年份作为 key_years，这些是财运变动关键年。若无财食流年，则选用神年。")
    lines.append("◆ marriage 婚姻：男命筛选十神为【正财/偏财】的流年；女命筛选十神为【正官/七杀】的流年。若无对应十神流年，则选大运交替节点年。")
    lines.append("◆ relationship 贵人：从流年中筛选十神为【正印/偏印】的年份，或用神年。若无印星流年，则选用神年。")
    lines.append("◆ health 健康：从流年中筛选标签为【忌神年⚠️】的年份作为 key_years，忌神年份代表运势低点和健康风险。若无忌神年，则选冲克日主的流年。")
    lines.append("")
    lines.append("⚠️ 关键约束：不同维度必须有不同的 key_years！不能所有维度都选用神年/忌神年。每个维度必须按其对应的十神类型独立选年。选完后在 summary 中引用典籍原文说明为何这些年份对该维度关键。")
    lines.append("")
...
async def generate_forecast(
        ...
        if ai_result:
            ...
            if all(dim in ai_result for dim in required_dims):
                ...
                DIM_TEN_GOD = {
                    "career": {"正官", "七杀", "正印", "偏印"},
                    "wealth": {"正财", "偏财", "食神", "伤官"},
                    "marriage": {"正财", "偏财", "正官", "七杀"},
                    "relationship": {"正印", "偏印"},
                    "health": set(),  # health 用忌神年，不按十神
                }
                for dim in required_dims:
                    if not forecast[dim].get("key_years"):
                        ...
                        for y in range(es2, du_ey2 + 1):
                            ...
                            liunian_fb.append({"year": y, "label": label, "ten_god": tg_data.get("ten_god", ""), "advice": tg_data.get("advice", "关注此年")})

                        target_gods = DIM_TEN_GOD.get(dim, set())
                        if dim == "health":
                            # 健康维度：优先忌神年
                            picks = [ln for ln in liunian_fb if "忌神" in ln.get("label", "")][:2]
                            if not picks:
                                picks = [ln for ln in liunian_fb if "用神" not in ln.get("label", "")][:2]
                        elif target_gods:
                            # 优先筛选目标十神年份，再辅助用神年
                            god_picks = [ln for ln in liunian_fb if ln.get("ten_god", "") in target_gods]
                            if god_picks:
                                picks = god_picks[:2]
                            else:
                                # 无目标十神流年，降级为用神年
                                picks = [ln for ln in liunian_fb if "用神" in ln.get("label", "")][:2]
                        else:
                            picks = [ln for ln in liunian_fb if "用神" in ln.get("label", "")][:2]
    ...
    for dim in ["career", "wealth", "marriage", "relationship", "health"]:
        ...
        mock_forecast[dim]["key_actions"] = {k: v for k, v in raw_actions.items()
            if (isinstance(k, int) and k >= es and k <= du_ey) or
            (isinstance(k, str) and any(int(p) >= es and int(p) <= du_ey for p in k.split() if p.isdigit()))}
        # Fallback for empty — 按维度分别选年（不再统一取 n//3, 2n//3）
        ...
        if not mock_forecast[dim].get("key_years") and len(du_years) >= 2:
            ...
            DIM_OFFSETS = {
                "career": (n//4, 3*n//4),
                "wealth": (n//5, 3*n//5),
                "marriage": (n//3, 2*n//3 + 1),
                "relationship": (n//3 + 1, 2*n//3 + 2),
                "health": (n//4 + 1, 3*n//4 + 1),
            }
            offsets = DIM_OFFSETS.get(dim, (n//3, 2*n//3))
            a, b = min(offsets[0], n-1), min(offsets[1], n-1)
            mock_forecast[dim]["key_years"] = [du_years[a], du_years[b]]
            mock_forecast[dim]["key_actions"] = {
                str(du_years[a]): "关注运势变化",
                str(du_years[b]): "把握关键机遇"
            }
