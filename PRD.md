...
> 创建：2026-07-19 | 状态：✅ 已完成 2026-07-19
...
├── LLM 复盘 → 活权重
├── 反馈→权重→检索的闭环
└── 补齐：Mock复盘规则逻辑V2（12条规则+5层面定向调整）

V2.3 ✅ 已完成 2026-07-19
├── SQLite 典籍库构建（3.6MB, 281章）
├── FTS5 全文检索
├── 8本典籍统一元数据
└── 补齐：穷通宝鉴精准日干过滤（ri_zhu_stem分级匹配+章节注入加分）

V2.4 ✅ 已完成 2026-07-19
├── 三命通会+渊海子平接入RAG（basics/shishen 阶段）
└── 补齐：反馈→校准联动（核心三关全挂→建议recalibrate_time）
